...
browser.storage.onChanged.addListener((changes, areaName) => {
  if (areaName === "local" && changes[PRIVATE_LAUNCH_KEY]) {
    showLaunch(changes[PRIVATE_LAUNCH_KEY].newValue);
  }
});
...
async function initialize() {
  try {
    ...
    if (!currentWindow.incognito) {
      throw new Error("This launcher only works in a private window");
    }
    if (!launchNonce) {
    ...
    if (!Number.isInteger(currentTab?.id) || !Number.isInteger(currentTab.windowId)) {
      throw new Error("Firefox did not provide the launcher tab details");
    }
    ...
    await browser.storage.local.set({[PRIVATE_LAUNCH_KEY]: readyLaunch});
